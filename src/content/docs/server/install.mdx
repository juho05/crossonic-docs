...
services:
  crossonic:
    ...
    volumes:
      ...
      # Alternatively, check https://crossonic.org/server/configuration/multi-library/ for how to
...
Change `/path/to/your/music` to the directory path where your music files are stored **OR**
visit the [multi library configuration guide](/server/configuration/multi-library)
to configure multiple libraries.
...
If everything went well you'll be able to access crossonic-server on port `8080` of your server.
