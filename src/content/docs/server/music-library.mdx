...
Of no artist image file is available for a given artist their image is fetched automatically from last.fm if [last.fm integration](/server/configuration/lastfm) is configured.