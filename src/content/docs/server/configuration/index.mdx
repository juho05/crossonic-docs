...
| `ENCRYPTION_KEY` | A secure random key to encrypt secrets in the database. Must be a base64 encoded string representing exactly 32 bytes. Generate with `docker run --rm -it --entrypoint crossonic-admin ghcr.io/juho05/crossonic-server gen-encryption-key` | `gLZQ4A1oqi+pPHfk7Q9A6mCpOGBKR2ZcVq94tHW0lKo` |
